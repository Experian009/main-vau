...
function Install-FFmpeg {
  ...
  # Resolve the real current asset name through the GitHub API. The release
  # does not expose a stable /latest/download/ffmpeg-release-essentials.zip
  # alias; its current name is versioned (for example 9.0.1).
  $asset = Get-LatestAsset "GyanD/codexffmpeg" "essentials_build\.zip$"
  Install-ZipAsset $asset $destination "ffmpeg.exe"
