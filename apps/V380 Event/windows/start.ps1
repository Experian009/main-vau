...
function Wait-TcpEndpoint([string]$hostName, [int]$port, [int]$timeoutSeconds = 45) {
  $deadline = (Get-Date).AddSeconds($timeoutSeconds)
  do {
    try {
      if (Test-NetConnection -ComputerName $hostName -Port $port -InformationLevel Quiet -WarningAction SilentlyContinue) { return $true }
    } catch { }
    Start-Sleep -Milliseconds 500
  } while ((Get-Date) -lt $deadline)
  return $false
...
$runtimeMediaConfig = Join-Path $RuntimeRoot "native-mediamtx.runtime.yml"
$webrtcHost = if ($env:WEBRTC_HOST) { $env:WEBRTC_HOST } else { "127.0.0.1" }
(Get-Content -LiteralPath $mediaConfig -Raw).Replace("webrtcAdditionalHosts: [127.0.0.1]", "webrtcAdditionalHosts: [$webrtcHost]") | Set-Content -LiteralPath $runtimeMediaConfig -Encoding UTF8
# Start-Process joins ArgumentList into one command line. Quote the config
# path explicitly because this project directory contains a space ("V380 Event").
$mediaProcess = Start-LoggedProcess "mediamtx" $mediamtx @("`"$runtimeMediaConfig`"") $ProjectRoot
if (-not (Wait-HttpEndpoint "http://127.0.0.1:8889/live/whep" 30)) {
  Write-Host "Последние сообщения MediaMTX:" -ForegroundColor Red
  Get-Content (Join-Path $LogsRoot "mediamtx.err.log") -Tail 80 -ErrorAction SilentlyContinue
  Get-Content (Join-Path $LogsRoot "mediamtx.out.log") -Tail 80 -ErrorAction SilentlyContinue
  throw "MediaMTX WebRTC API не запустился на 127.0.0.1:8889. Проверьте .runtime\windows\logs\mediamtx.err.log"
}
if (-not (Wait-TcpEndpoint "127.0.0.1" 8554 30)) {
  throw "RTSP decoder endpoint не доступен на 127.0.0.1:8554. Проверьте .runtime\windows\logs\decoder.err.log"
}
