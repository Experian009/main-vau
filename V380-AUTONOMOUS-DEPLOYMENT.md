...
6. После входа пользователя в удалённый Windows рабочий стол Scheduled Task запускает `windows/start.ps1`; тот поднимает decoder, MediaMTX, gateway, Telegram notifier, motion detector, event archive, continuous recorder, continuous archive и web UI. Задача работает от имени интерактивного пользователя, а не `SYSTEM`, поэтому процессам доступен тот же профиль и рабочий контекст, что и при ручном запуске.

CI-команда с `-Install -Register` только устанавливает компоненты и регистрирует задачу; она не запускает сервисы внутри GitHub Actions. Это предотвращает их завершение во время очистки runner. Реальный запуск выполняется после входа в рабочий стол. Если первая установка завершилась после установки Bun, но до установки FFmpeg, Scheduled Task обнаруживает отсутствующий `.runtime/windows/ffmpeg/ffmpeg.exe`, повторно выполняет setup и продолжает запуск автоматически. Имя FFmpeg-архива определяется через GitHub API, поэтому изменение версии не приводит к 404 из-за устаревшего имени файла.
