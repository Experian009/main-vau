...
## Автономный V380 Event MVP (авто-развёртывание на Windows-раннере)

Workflow `.github/workflows/main.yml` теперь при каждом запуске (`workflow_dispatch`)
сам поднимает VPS-камера-сервис `v380-event/` — без единого действия внутри
самой VM: RDP включается, сервис ставится и стартует, дашборд пробрасывается
наружу через отдельный туннель bore.

**Разовая настройка (только один раз, перед первым запуском):**

1. В настройках репозитория → `Settings → Secrets and variables → Actions` добавьте секреты:

| Секрет | Обязателен | Назначение |
|---|---|---|
| `WIN_PASS` | да | пароль учётки `runneradmin` для RDP (уже было в проекте) |
| `V380_CAMERA_ID` | да | ID вашей камеры V380 |
| `V380_PASSWORD` | да | пароль камеры |
| `GATEWAY_TOKEN` | да | произвольный длинный токен для защиты gateway API |
| `V380_USERNAME` | нет (по умолчанию `admin`) | логин камеры |
| `INTERNAL_NOTIFY_TOKEN` | нет (генерируется автоматически, если не задан) | внутренний токен notifier-сервиса |
| `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` | нет | уведомления о событиях в Telegram (нужны оба сразу, иначе игнорируются) |
| `TERABOX_NDUS` | нет | загрузка архива событий/записи в TeraBox |

2. Запустите workflow **Windows-RDP-BORE** (вкладка Actions → Run workflow).

**Что происходит дальше полностью автоматически, без участия человека:**
1. Раннер поднимает Windows Server VM, включается RDP.
2. Стартует туннель bore для RDP (3389).
3. `v380-event/windows/unattended-deploy.ps1` генерирует `.env.windows` из секретов,
   ставит FFmpeg при необходимости, скачивает V380Decoder/MediaMTX/Bun и запускает
   все сервисы (decoder, mediamtx, gateway, notifier, motion, archive, continuous,
   continuous-archive, web) в фоне.
4. Поднимается второй туннель bore для веб-интерфейса (порт 3000) — адрес появится
   в логах шага **Start bore tunnel for V380 dashboard**.
5. Job уходит в keep-alive до истечения 6-часового лимита раннера.

Логи сервисов V380 доступны через RDP: `v380-event\windows\logs.cmd` или
`v380-event\.runtime\windows\logs\*.log`.

⚠️ **Важно:**
- Файл `.env.windows` **никогда не коммитится** — он создаётся заново на раннере
  при каждом запуске из секретов и живёт только на эфемерной VM.
- Использование GitHub Actions как постоянного VPS/RDP-хостинга (что и делает
  этот репозиторий) выходит за рамки предполагаемого использования Actions
  (CI/CD) и формально может нарушать Acceptable Use Policies GitHub — вплоть до
  блокировки аккаунта при частом/длительном использовании. Это ограничение
  самого GitHub, а не этого скрипта.
- VM полностью эфемерна: при остановке job (вручную или по таймауту 6 часов)
  все локальные записи и события на ней пропадают безвозвратно. Если нужна
  сохранность записей — используйте TeraBox-архивацию (уже встроена) или
  выгружайте `local-data/` до завершения job.
