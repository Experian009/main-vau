...
## Autonomous V380 Event MVP on Windows

The repository now contains the complete V380 Event MVP under `apps/V380 Event`. Both Windows workflows check out the repository, provision the V380 runtime, create the protected `.env.windows` file from GitHub Actions secrets, register a Windows Scheduled Task, and start the camera service automatically.

The task is named `V380 Event MVP - Autonomous Startup`. It runs as `SYSTEM` at Windows startup with a short random delay, waits for network connectivity, starts `windows/start.ps1`, and is configured to restart after failures. Logs are written to `apps/V380 Event/.runtime/windows/autonomous-bootstrap.log` and the normal service logs remain under `.runtime/windows/logs`.

Add these repository secrets before running either Windows workflow:

| Secret | Purpose |
|---|---|
| `WIN_PASS` | Windows RDP account password. |
| `V380_CAMERA_ID` | V380 camera identifier. |
| `V380_USERNAME` | Camera username; usually `admin`. |
| `V380_PASSWORD` | Camera password. |
| `GATEWAY_TOKEN` | Browser-to-gateway authentication token. |
| `INTERNAL_NOTIFY_TOKEN` | Internal motion-to-Telegram authentication token. |
| `TELEGRAM_BOT_TOKEN` | Optional Telegram bot token. |
| `TELEGRAM_CHAT_ID` | Optional Telegram destination chat. |
| `TERABOX_NDUS` | Optional TeraBox `ndus` cookie. |
| `TS_AUTHKEY` | Required only by the Tailscale workflow. |

The camera password, gateway token and TeraBox cookie are never printed by the bootstrap script. They are written only to the VM's local `.env.windows` file. Do not commit that file.

The GitHub-hosted Windows runner is temporary and follows the repository's existing execution limit. The autonomous task starts the service after each Windows boot during that runner session; it cannot survive destruction of the runner itself.
