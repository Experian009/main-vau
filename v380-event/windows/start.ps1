...
if (-not (Get-Command $ffmpeg -ErrorAction SilentlyContinue)) { throw "FFmpeg не найден. Установите ffmpeg.exe и добавьте его в PATH или укажите FFMPEG_BIN в .env.windows" }
...
try {
  # На headless-раннере (без интерактивной сессии/дефолтного браузера) это
  # может упасть — сервисы уже подняты и это не должно останавливать скрипт.
  Start-Process "http://localhost:3000" -ErrorAction Stop
} catch {
  Write-Host "Не удалось открыть браузер автоматически (нормально для headless-окружения): $($_.Exception.Message)" -ForegroundColor Yellow
}
